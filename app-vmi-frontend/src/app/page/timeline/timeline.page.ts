...
export class TimelinePage implements OnInit {
  ...
  constructor(
    ...
    public timelineService: HttpService,
  ...
  loadData(event) {
    ...
    if (this.pageselected <= this.page) {

      this.getData(null, this.pageselected, 10);
    }
    event.target.complete();
  }

  openAddFeed(): void {
    this.router.navigate(['timeline-create']);
  }
  ...
  presentAlertDelete(id) {
    this.timelineService.deleteTutorial(id, (data) => {
      this.timelineService.getEquipmentsAdmin("1", "1000", (t) => {
        this.getData(null, 1, 10),
          this.equipments = this.timelineService.resultEquipAdmin;
      });
    });
  }

  openFile(id, full) {
    this.timelineService.getAttatchment(id, true, "timeline", (data) => {
    });
  }

  showThumbs(attachments, full, timeline) {
    IMAGE.thumbs = [];
    this.i = 0;
    this.thumb = [];
    this.timelineselect = timeline;
